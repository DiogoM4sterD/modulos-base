...
export class ColorPaletteSettings extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static PARTS = {
      form: {
        template: `modules/token-color-marker/templates/color-palette-settings.hbs`,
    ...
    async _handleButtonClick(event) {
      ...
      switch (action) {
        case 'create': {
          await ColorPaletteSettings.createColor();
          this.render({isScrollDown: true});
          break;
        }
  
        case 'add-image': {
          await ColorPaletteSettings.addImage();
          this.render({isScrollDown: true});
          break;
        }
        ...
        case 'delete': {
            ...
            if (confirmed) {
              await ColorPaletteSettings.deleteColor(colorId);
              this.render();
            }
        ...
        case 'move-up': {
            await ColorPaletteSettings.moveColor(index, index-1);
            this.render();
            break;
        }
    ...
    async _prepareContext(options) {
      ...
      const colors = ColorPaletteSettings.getColors();
    ...
    async _onChangeInput(event) {
      ...
      await ColorPaletteSettings.updateColor(inputName[0], inputName[1], event.currentTarget.value);
