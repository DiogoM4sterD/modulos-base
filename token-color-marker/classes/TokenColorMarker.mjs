...
export class TokenColorMarker {
    ...
    static addTokenColorMarkerModule() {
        Hooks.on('renderTokenHUD', (app, html, data) => { 
            ...
            // First remove existing to prevent multiple registers when clicking token visibility
            $(html).off('click', `.control-icon[data-palette="${MODULENAME}"]`);
            $(html).on('click', `.control-icon[data-palette="${MODULENAME}"]`, (event) => {
            ...
            $(html).on('click', '.control-icon[data-palette="effects"]', (event) => {
                this.deactivateTokenColorMarkerButton($(event.currentTarget));
                // remove flag so the palette does not show when selecting effects
                app[FLAGS.COLORMARKERCLASS] = "";
                // removed the color from showing up in the effects palette
                canvas.hud.token.render(true);

            });

            // register click event listener for movementActions button
            $(html).on('click', '.control-icon[data-palette="movementActions"]', (event) => {
                this.deactivateTokenColorMarkerButton($(event.currentTarget));
                // remove flag so the palette does not show when selecting movement
                app[FLAGS.COLORMARKERCLASS] = "";
            });
    ...
    static addTokenColorMarkerUI(tokenHUD, html, data) {
        ...
        rightButtonColumn.prepend(
            `<button type="button" class="control-icon ${active ?? ''}" data-action="${MODULENAME}" data-palette="${MODULENAME}">
                <img src="${TokenColorMarker.DefaultIcon}" width="36" height="36" title="${buttonTooltip}" ${style}>
            </button>
            <div class="palette ${MODULENAME}-palette ${active ?? ''}">
                ${markers}
    ...
    static activateTokenColorMarkerButton(colorMarkerButton, tokenHUD) {
        ...
        colorMarkerButton[0].form.querySelector(`.${MODULENAME}-palette`).classList.toggle("active");

        // Deactivate the effects button and status effects palette
        tokenHUD.togglePalette("effects", false);
        tokenHUD.togglePalette("movementActions", false);
	}
    ...
    static async deactivateTokenColorMarkerButton(effectsButton) {
        ...
        const palette = colorMarkerButton.form.querySelector(`.${MODULENAME}-palette`);
